...
from newglico.glico_model.model import Classifier
from newglico.glico_model.interpolate import save_inter_imgs
import newglico.glico_model.model as model
from newglico.glico_model.utils import *
from newglico.logger import Logger, savefig


class NAG():
    def __init__(self, nag_params, image_params, rn, resume, num_classes, resolution, datasets=None, is_ft=False):
        print("Class NAG __init__()")
        ...
        if self.VERBOSE: print(f"[DEBUG] dataset name: {str.lower(str(datasets[0].__class__))},offset_labels={offset_labels}")
        ...
        if self.BEN_COMMENTS:  print("\n[BEN] load to GPU \n")
        if self.num_gpu > 1:
            if self.VERBOSE: print(f"=>Using {self.num_gpu} GPUs!")
    ...
    def load_models(self):
        try:
            if self.VERBOSE: print(f"=> Loading model from {self.rn}")
            ...
            if self.epoch > 0:
                if self.VERBOSE: print(f"=> Loaded successfully! epoch:{self.epoch}")
                if self.is_ft:
                    self.netZ.ft_maps_from_loader()
            else:
                if self.VERBOSE: print("=> No checkpoint to resume")
                self.init_models_weights()

        except Exception as e:
            if self.VERBOSE: print(f"=>Failed resume job!\n {e}")
            self.init_models_weights()
    ...
    def check_consecutive(self, batch_size):
        if self.consecutive_loss > 10:
            self.consecutive_loss = 0
            if self.VERBOSE: print("=>Update batch_size")
    ...
    def train_long(self, train_data, unlabeled_train_data, test_data_transductive, opt_params, resolution,
                   vis_epochs=50, save_ckpt=1000):
        print("Class NAG train_long()")
        ...
        while epoch < opt_params.epochs:
            ...
            if self.VERBOSE: print(f"=>epoch:{epoch}/{opt_params.epochs}  resolution:{resolution} batch:{batch_size}, lr{lr:e}")
            ...
            if self.VERBOSE: print(f"NAG Epoch: {epoch} Error: {epoch_loss}, Time: {(time.time() - end_epoch_time) / 60:8.2f}m")
            ...
            if epoch_loss < self.best_loss:
                ...
                if self.VERBOSE: print(f"=>consecutive_loss: {self.consecutive_loss}")
    ...
    def visualize_no_module(self, epoch, aug_param, imgs):
        ...
        # print("self.vis_n",self.vis_n)
        # print("Irec.data.shape",Irec.data.shape)
...
class NAGTrainer():
    def __init__(self, dataset, nag_params, rn, resume=False, num_classes=10):
        ...
        if self.verbose: print("[NEW CODE] image_params:{}".format(self.image_params))
