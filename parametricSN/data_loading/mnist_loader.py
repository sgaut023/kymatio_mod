"""Wrapper for subsampling the MNIST based on given input parameters

Author: Laurent Alsene-Racicot and Benjamin Therien
...
Functions:
    mnist_getDataloaders -- samples from the MNIST dataset based on input
    mnist_augmentationFactory -- returns different augmentations for MNIST
...
def mnist_augmentationFactory(augmentation):
...
def mnist_getDataloaders(trainSampleNum, valSampleNum, trainBatchSize, 
                         valBatchSize, trainAugmentation, dataDir="."):
    """Samples a specified class balanced number of samples form the MNIST dataset
    
    returns:
        train_loader, test_loader, seed, glico_dataset
    """
    
    transform_train = mnist_augmentationFactory(trainAugmentation)
    transform_val = mnist_augmentationFactory("noaugment")

    dataset_train = datasets.MNIST(#load train dataset
        root=dataDir, train=True, 
        transform=transform_train, download=True
    )

    dataset_val = datasets.MNIST(#load test dataset
