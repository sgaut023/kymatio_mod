...
def create_scatteringExclusive(J,N,M,second_order,device,initialization,pixelwise=False,
    ...
    if initialization == "Tight-Frame":
        params_filters = create_filters_params(J,L,requires_grad, equivariance,pixelwise, device) #kymatio init
    elif initialization == "Random":
        params_filters = create_filters_params_random(J,L,requires_grad,equivariance,pixelwise,device) #random init
    else:
...
class sn_ScatteringBase(nn.Module):
    ...
    def __init__(self, J, N, M, second_order, initialization, seed, 
        ...
        self.scattering, self.psi, self.wavelets, self.params_filters, self.n_coefficients, self.grid = create_scatteringExclusive(
            J,N,M,second_order, initialization=self.initialization, equivariance=equivariance,pixelwise=self.pixelwise,
        ...
        if self.monitor_filters == True:
            _, self.compared_psi, self.compared_wavelets, self.compared_params, _, _ = create_scatteringExclusive(
                J,N,M,second_order, initialization='Tight-Frame',equivariance=equivariance,pixelwise=pixelwise,
